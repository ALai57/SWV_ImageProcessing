% Save images to the same directory as the DICOM directory. The types of
% images that can be saved are listed below. For image type 4, it is the
% original dataSWV map


function saveImages(M,type,folder)
% M is the cell array
...
% Create the tag for the end of the image filename
switch type
    case 1
        field = 'Original';
    case 2
...
for i=1:m
    ...
    name = M{i,3};
    switch imtype
        case 'region' % Can save any of the five options
            switch type
                case 1
                    saveo(M{i,4},name);
                case 2
        ...
        case 'panoramic' % As of now, only one option
            imwrite(M{i,4},strcat(name,'_pan.jpg'),'jpg');
